...
from new_leg_detection.msg import *
...
class LegDetection:
    ...
    def __init__(self):   #this is constructor
        ...
        rospy.Subscriber("/scan", LaserScan, self.getLaserScanCb, None, 10 ) #laser e paEni madde nazar ast
        ...
        s = rospy.Service('crowd_ack', crowd_srv, self.server_handler, 21) #kam morede Stefade qara gereft, faqat vase start e crowd_leg_detectin
        self.crowdPub = rospy.Publisher('/legs_crowd', legmsg)
        self.crowdPub2 = rospy.Publisher('/legs_crowd2', Legs)
        self.msg = legmsg()
        ...
        self.markerPublisher = rospy.Publisher("/LegMarker", visualization_msgs.msg.MarkerArray, queue_size = 10)    
        
        self.legPublisherScenario = rospy.Publisher('/LegDetected', std_msgs.msg.Bool, queue_size = 10) #for scenario    
...
#             self.downLaserCb(laser, self.globalFirstPoint, self.globalEndPoint)
    ...
    def downLaserCb(self, laser, First, End):    
        if self.legPublisher.get_num_connections() > 0 or self.crowdPub2.get_num_connections():   
            ...
            while i < len(angle):            
                ...
                if (len(angle[i]) - cmp2) < ((30.0 * len(angle[i])) / 100): #yani dar suraT varede if mishavad k ekhtelafe cmp2 ba har goodcluster bayad kamtar az 20% an bashad                                
            ...
            while i < len(clustersDistance):   #good distance togetherLegs choose
                if clustersDistance[i] > self.togetherMin and clustersDistance[i] < self.togetherMax:
            ...
            while i < len(goodMarkersTogetherLegs):
                ...
                if len(string[i]) == 4:    #this mean LRLR number is 4            
                    if string[i][0] == "L" and string[i][1] == "R" and string[i][2] == "L" and string[i][3] == "R":                  
                ...
                while k < (int(len(clustersPoints[gmtl]) - 1) / 2) - 1:    #baraye inke avvali va akhari va vasaty dar bala hesab shode and.                                                       
                    ...
                    angl11 = numpy.arccos((pow(dist1[i][k], 2) + pow(dist2[i][k], 2) - pow(dist3[i], 2)) / (2 * dist1[i][k] * dist2[i][k]) )
                    angl11 = (angl11 * (180 / pi)) #baraye tabDle radian b daraje
                    angl22 = numpy.arccos((pow(dist5[i][k], 2) + pow(dist6[i][k], 2) - pow(dist4[i], 2)) / (2 * dist5[i][k] * dist6[i][k]) )
            ...
            while i < len(angle11):            
                ...
                while j < len(angle11[i]) - 1:
                    ...
                    if (abs(angle11[i][j] - angle11[i][j + 1]) < 25) and (abs(angle11[i][j] - self.threshold_angle) < 25) and (abs(angle22[i][j] - angle22[i][j + 1]) < 25) and (abs(angle22[i][j] - self.threshold_angle) < 25) :                                       
                        cmp2 = cmp2 + 1                                        
                                                
                    if ((len(angle11) - cmp3) == 0) and (abs(angle11[i][j + 1] - self.threshold_angle) < 25) and ((abs(angle22[i][j + 1] - self.threshold_angle) < 25)): #baraye inke zaviye akhar ham barresi shavad
                        cmp2 = cmp2 + 1                                                                                   
                ...
                if (len(angle11[i]) - cmp2) < ((40.0 * len(angle11[i])) / 100) and (len(angle22[i]) - cmp2) < ((40.0 * len(angle22[i])) / 100): #yani dar suraT varede if mishavad k ekhtelafe cmp2 ba har goodcluster bayad kamtar az 20% an bashad # va agar dar 20.0 ".0" nabashad javab ra float bar nmigardanad, balke b surat int mikonad.                                
            ...
            while i2 < len(togetherLegs):
                 ...
                 #x2.append([])
                 #y2.append([])
                 lif = clustersPoints[togetherLegs[i2]][0] # LIF means, is, Legs Indexes First                       
